...
if ( ! function_exists ( 'jumtechWP_setup' ) ) {
	...
	function jumtechWP_setup() {
		...
		register_nav_menus( array(
			'primary' => __( 'Primary Menu', 'jumtechWP' ),
			'footer-menu' => __( 'Footer Menu', 'jumtechWP' ),
		) );
...
if ( ! function_exists( 'jumtechWP_all_excerpts_get_more_link' ) ) {
	...
	function jumtechWP_all_excerpts_get_more_link( $post_excerpt ) {
		if ( ! is_admin() ) {
			$post_excerpt = $post_excerpt . ' [...]<p><a class="btn btn-secondary jumtechWP-read-more-link" href="' . esc_url( get_permalink( get_the_ID() ) ) . '">' . __( 'Read More...',
			'jumtechWP' ) . '</a></p>';
		}
		return $post_excerpt;
	}
